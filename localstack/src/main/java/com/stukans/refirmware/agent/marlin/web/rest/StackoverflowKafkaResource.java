...
@RestController
@RequestMapping("/api/stackoverflow-kafka")
public class StackoverflowKafkaResource {

    private final Logger log = LoggerFactory.getLogger(StackoverflowKafkaResource.class);

    private final MessageChannel output;
    private Sinks.Many<Message<String>> sink = Sinks.many().unicast().onBackpressureBuffer();

    public StackoverflowKafkaResource(@Qualifier(KafkaSseProducer.CHANNELNAME) MessageChannel output) {
        this.output = output;
    }
