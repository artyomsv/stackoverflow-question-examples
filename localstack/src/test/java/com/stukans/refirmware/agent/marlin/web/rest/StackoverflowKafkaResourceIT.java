...
import com.stukans.refirmware.agent.marlin.IntegrationTest;
import com.stukans.refirmware.agent.marlin.config.EmbeddedKafka;
import com.stukans.refirmware.agent.marlin.config.KafkaSseConsumer;
import com.stukans.refirmware.agent.marlin.config.KafkaSseProducer;
...
@WithMockUser
@EmbeddedKafka
class StackoverflowKafkaResourceIT {
    ...
    void producesMessages() throws InterruptedException {
        client.post().uri("/api/stackoverflow-kafka/publish?message=value-produce").exchange().expectStatus().isNoContent();
    ...
    void consumesMessages() {
        ...
        String value = client
            .get()
            .uri("/api/stackoverflow-kafka/consume")
